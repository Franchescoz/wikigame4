      ...
      <div className="min-h-screen flex items-center justify-center bg-[url('/fondoSesion.png')] bg-cover bg-center font-montserrat">
        <form className="relative bg-night w-[95%] max-w-screen-xl rounded-xl p-6 flex flex-col md:flex-row gap-6 shadow-2xl" onSubmit={(e)=>handleModificar(juegoN.id,e)}>
          <div className="flex flex-col items-center gap-4 w-full md:w-1/3">
            ...
            <button type="button" onClick={() => document.getElementById("uploadImage").click()}>Subir</button>
          </div>
          <div className="flex flex-col gap-3 w-full md:w-2/3 text-white">
            <input value={juegoN.titulo} onChange={e=>setJuegoN({...juegoN,titulo:e.target.value})}/>
            <input value={juegoN.consolas} onChange={e=>setJuegoN({...juegoN,consolas:e.target.value})}/>
            <input value={juegoN.tipo_juego} onChange={e=>setJuegoN({...juegoN,tipo_juego:e.target.value})}/>
            <textarea value={juegoN.descripcion} onChange={e=>setJuegoN({...juegoN,descripcion:e.target.value})}/>
          </div>
        </form>
      </div>
    );
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[url('/fondoSesion.png')] bg-cover bg-center font-montserrat">
      <div className="relative bg-night w-[95%] max-w-screen-xl rounded-xl p-6 flex flex-col md:flex-row gap-6 shadow-2xl min-h-[40rem] text-white">
        ...
        <div className="flex flex-col items-center gap-4 w-full md:w-1/3">
          <img src={`${supabaseURL}/storage/v1/object/public/images/${juegoN.image_juego?.[fotoPrincipal]?.image_url}`} className="w-56 h-80 rounded-lg object-cover border-2 border-Lavanda"/>
          <div className="grid grid-cols-4 gap-2 w-56">
            {[0,1,2,3].map((index)=>(
              <div key={index} onClick={()=>juegoN.image_juego?.[index] && setFotoPrincipal(index)}>
                {juegoN.image_juego?.[index] && (
                  <img src={`${supabaseURL}/storage/v1/object/public/images/${juegoN.image_juego[index].image_url}`} className="w-12 h-12 object-cover"/>
                )}
              </div>
            ))}
          </div>
        ...
        <div className="flex flex-col gap-3 w-full md:w-2/3">
          <h1>{juegoN.titulo}</h1>
          <p><b>Consolas:</b> {juegoN.consolas}</p>
          <p><b>Tipo:</b> {juegoN.tipo_juego}</p>
