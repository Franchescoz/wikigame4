  ...
  return (
    <div className="min-h-screen bg-cover bg-center bg-[url('/fondoSesion.png')] flex items-center justify-center">
      <form onSubmit={registrarUsuario} className="bg-night bg-opacity-95 p-8 sm:p-14 rounded-3xl items-center space-y-6 w-4/5 sm:w-[600px] lg:w-[700px]">
        ...
        <input type="text" placeholder="@ Usuario" value={nombre} onChange={(e) => setNombre(e.target.value)} required className="w-full p-4 rounded-md text-black bg-white outline-none" />
        ...
        <input type="email" placeholder="✉️ Correo" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full p-4 rounded-md text-black bg-white outline-none" />
        <p className="text-white self-start text-sm">{email.includes("@") ? "Correo válido" : "Te falta el @ en el correo"}</p>

        <input type="password" placeholder="🔒 Contraseña" value={contraseña} onChange={(e) => setContraseña(e.target.value)} required minLength={6} pattern="(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{6,}" className="w-full p-4 rounded-md text-black bg-white outline-none" />
        ...
        <div className="flex flex-col sm:flex-row gap-6 mt-6 w-full">
          <button type="submit" className="w-full sm:w-1/2 bg-Lavanda py-3 rounded-full font-medulaone text-white font-bold text-xl hover:bg-purple-700 transition">
            Registrar
          </button>
